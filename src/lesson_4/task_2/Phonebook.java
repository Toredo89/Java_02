    ...
    public void add(String surname, String number) {
        if(book.containsKey(surname)) {
            ...
            if(!numbers.contains(number)) {
                numbers.add(number);
                System.out.println(String.format("Номер %s добавлен для фимилии %s", number, surname));
            } else {
                System.out.println(String.format("Номер %s уже существует для фимилии %s", number, surname));
            }
        } else {
